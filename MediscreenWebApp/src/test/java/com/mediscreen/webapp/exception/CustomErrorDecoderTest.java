...
class CustomErrorDecoderTest {
    ...
    void decode409Test() {
        ...
        Exception exception = customErrorDecoder.decode("PatientManagementProxy#getPatientById(Integer)", response);
        assertThat(exception.getMessage()).isEqualTo("Un patient existe déjà avec ces mêmes données clé (patientId 20)");
        assertThat(exception).isInstanceOf(AlreadyExistException.class);
